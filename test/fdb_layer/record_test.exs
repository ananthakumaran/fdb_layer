defmodule FDBLayer.RecordTest do
  ...
  test "crud" do
    ...
    assert_raise FDBLayer.DuplicateRecordError, fn ->
      Database.transact(db, fn t ->
        Repo.create(t, post_record, %Post{id: "1234", user_id: "9", claps: 0, content: "hello"})
      end)
    end
  ...
  test "version index" do
    ...
    Database.transact(db, fn t ->
      post = Repo.get(t, post_record, "1234")
      Repo.update(t, post_record, %{post | content: "world"})
    end)
    ...
    changes =
      Index.scan(
        Store.index(store, Post, "posts_changes"),
        db,
